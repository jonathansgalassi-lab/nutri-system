...
exports.acompanhamentoRouter = void 0;
exports.verificarAlertas = verificarAlertas;
exports.enviarCheckinsSemana = enviarCheckinsSemana;
exports.responderFeedback = responderFeedback;
const express_1 = require("express");
const connection_1 = require("../../database/connection");
const twilio_1 = require("../../integrations/twilio");
const date_fns_1 = require("date-fns");
exports.acompanhamentoRouter = (0, express_1.Router)();
// ─── Templates de check-in ────────────────────────────────────
const CHECKINS_ROTATIVOS = [
    // Semana 1 — Feedback geral
    (nome) => `Oi ${nome}! 👋 Como foi sua semana com o plano?
...
(Responde com o número)`,
    ...
    (nome) => `Oi ${nome}! 🍽️ Alguma refeição que você não tá curtindo?
...
5️⃣ Quero trocar várias refeições`,
    ...
    (nome) => `Oi ${nome}! Você tá tendo alguma dificuldade? 🤔
...
5️⃣ Outro motivo`,
    ...
    (nome) => `Oi ${nome}! Já se passou 1 mês! 🎉
...
async function responderFeedback(whatsapp, pacienteId, score, semanaCheckin) {
    let msg = '';
    ...
    if (msg)
        await (0, twilio_1.enviarWhatsApp)(whatsapp, msg);
    // Salva o check-in
    await (0, connection_1.query)(`INSERT INTO checkins (paciente_id, semana_numero, score_aderencia, respondido_em)
     VALUES ($1, $2, $3, NOW())
     ON CONFLICT DO NOTHING`, [pacienteId, semanaCheckin, score]);
    // Verifica alertas
    await verificarAlertas(pacienteId, whatsapp);
}
// ─── Sistema de alertas ───────────────────────────────────────
async function verificarAlertas(pacienteId, whatsapp) {
    const nutriWpp = process.env.NUTRICIONISTA_WHATSAPP;
    if (!nutriWpp)
        return;
    const paciente = await (0, connection_1.queryOne)('SELECT * FROM pacientes WHERE id = $1', [pacienteId]);
    if (!paciente)
        return;
    // 🚨 ALERTA VERMELHO: sem resposta há 14+ dias
    const ultimoCheckin = await (0, connection_1.queryOne)(`SELECT respondido_em FROM checkins
     WHERE paciente_id = $1 AND respondido_em IS NOT NULL
     ORDER BY respondido_em DESC LIMIT 1`, [pacienteId]);
    if (!ultimoCheckin)
        return;
    const diasSemResposta = Math.floor((Date.now() - new Date(ultimoCheckin.respondido_em).getTime()) / 86_400_000);
    if (diasSemResposta >= 14) {
        await (0, twilio_1.enviarWhatsApp)(nutriWpp, `🚨 *ALERTA VERMELHO*\n\n${paciente.nome} está sem responder há ${diasSemResposta} dias.\n📱 ${paciente.whatsapp}`);
        return;
    }
    // ⚠️ ALERTA AMARELO: score 1-2 por 2+ semanas consecutivas
    const ultimosCheckins = await (0, connection_1.query)(`SELECT * FROM checkins
     WHERE paciente_id = $1
     ORDER BY semana_numero DESC LIMIT 2`, [pacienteId]);
    if (ultimosCheckins.length === 2 &&
        ultimosCheckins.every((c) => c.score_aderencia <= 2)) {
        await (0, twilio_1.enviarWhatsApp)(nutriWpp, `⚠️ *ALERTA AMARELO*\n\n${paciente.nome} está com score baixo (≤2) por 2 semanas seguidas.\nConsidere revisar o plano.\n📱 ${paciente.whatsapp}`);
        return;
    }
    // ✅ ALERTA VERDE: score 4-5 por 3+ semanas
    const topCheckins = await (0, connection_1.query)(`SELECT * FROM checkins
     WHERE paciente_id = $1
     ORDER BY semana_numero DESC LIMIT 3`, [pacienteId]);
    if (topCheckins.length === 3 &&
        topCheckins.every((c) => c.score_aderencia >= 4)) {
        await (0, twilio_1.enviarWhatsApp)(nutriWpp, `✅ *ALERTA VERDE*\n\n${paciente.nome} está com aderência excelente por 3 semanas seguidas! 🎉\n📱 ${paciente.whatsapp}`);
    }
}
// ─── Job semanal (toda segunda às 08h) ───────────────────────
async function enviarCheckinsSemana() {
    // Busca pacientes ativos com pelo menos uma consulta realizada
    const pacientes = await (0, connection_1.query)(`SELECT DISTINCT p.*, c.data_hora AS data_consulta
     FROM pacientes p
    ...
    for (const paciente of pacientes) {
        const semanaAtual = (0, date_fns_1.differenceInWeeks)(new Date(), new Date(paciente.data_consulta)) + 1;
        const tipoCheckin = (semanaAtual - 1) % 4; // 0, 1, 2 ou 3
        const templateFn = CHECKINS_ROTATIVOS[tipoCheckin];
        const primeiroNome = paciente.nome.split(' ')[0];
        try {
            await (0, twilio_1.enviarWhatsApp)(paciente.whatsapp, templateFn(primeiroNome));
...
exports.acompanhamentoRouter.post('/resposta', async (req, res) => {
    const { paciente_id, score, semana_numero, dificuldades } = req.body;
    ...
    await (0, connection_1.query)(`INSERT INTO checkins (paciente_id, semana_numero, score_aderencia, dificuldades, respondido_em)
     VALUES ($1, $2, $3, $4, NOW())`, [paciente_id, semana_numero, score, dificuldades ?? null]);
...
exports.acompanhamentoRouter.get('/:paciente_id', async (req, res) => {
    const { paciente_id } = req.params;
    const checkins = await (0, connection_1.query)(`SELECT * FROM checkins WHERE paciente_id = $1 ORDER BY semana_numero ASC`, [paciente_id]);
    res.json({ checkins });
});
