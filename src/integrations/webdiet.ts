...
function formatarPlanoAnamnese(plano: ConteudoPlano, dados: DadosPacienteWebdiet): string {
...
async function buscarEAbrirPaciente(page: Page, nome: string, telefone?: string): Promise<boolean> {
  ...
  if (achou) {
    ...
    await page.evaluate(() => {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      if ((window as any).Swal) { (window as any).Swal.close(); return; }
      // Fallback: botão específico do SweetAlert2 ou texto "não registrar"
      const btn = document.querySelector<HTMLElement>('.swal2-cancel, .swal2-deny')
