...
async function criarPrescricaoAlimentar(page: Page, dados: DadosPacienteWebdiet): Promise<void> {
  ...
  try {
    ...
    if (nomePosto) console.log(`[webdiet] Nome da prescrição preenchido: "${nomePrescricao}"`);
    ...
    await page.evaluate((kcal) => {
      ...
      for (const sel of sels) {
        const el = document.querySelector<HTMLInputElement>(sel);
        if (el) { el.value = String(kcal); el.dispatchEvent(new Event('input', { bubbles: true })); return; }
      }
    ...
    for (const { chave, nome, horario } of refeicoesCfg) {
      ...
      for (let i = 0; i < Math.min(opcoes.length, 3); i++) {
        ...
        await page.evaluate((texto) => {
          ...
          if (lastTa) {
            lastTa.focus(); lastTa.value = texto;
            lastTa.dispatchEvent(new Event('input', { bubbles: true }));
            return;
          }
          // Último input de texto vazio
          const inputs = Array.from(document.querySelectorAll<HTMLInputElement>('input[type="text"]:not([readonly])'));
          const lastInput = inputs.filter(i => !i.value)[inputs.length - 1];
          if (lastInput) {
            lastInput.focus(); lastInput.value = texto;
    ...
    if (plano.recomendacoes?.length || plano.alertas_nutricionista?.length) {
      ...
      await page.evaluate((obs) => {
        ...
        for (const sel of sels) {
          ...
          if (el) { el.value = obs; el.dispatchEvent(new Event('input', { bubbles: true })); return; }
        }
    ...
    if (!salvou) await esperarEClicar(page, 'confirmar', false, 3000);
    await new Promise(r => setTimeout(r, 2000));
