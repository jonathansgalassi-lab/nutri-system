-- ============================================================
-- NUTRI-SYSTEM — Schema do Banco de Dados
-- ============================================================
...
CREATE TABLE IF NOT EXISTS planos_alimentares (
  ...
  status VARCHAR(20) DEFAULT 'rascunho', -- rascunho | aprovado | ativo
...
CREATE TABLE IF NOT EXISTS consultas (
  ...
  tipo VARCHAR(50), -- inicial | reavaliacao | online | presencial
  local VARCHAR(200),
...
CREATE TABLE IF NOT EXISTS contratos (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  paciente_id UUID REFERENCES pacientes(id) ON DELETE CASCADE,
  plano VARCHAR(50), -- inicial | acompanhamento | premium
  valor DECIMAL(10,2),
  data_inicio DATE,
  data_fim DATE,
  forma_pagamento VARCHAR(20),
  status VARCHAR(20), -- ativo | expirado | cancelado
  created_at TIMESTAMP DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS pagamentos (
  ...
  status VARCHAR(20) -- pago | pendente | cancelado
);
